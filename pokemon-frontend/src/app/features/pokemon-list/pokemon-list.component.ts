...
export class PokemonListComponent implements OnInit {
  ...
  loadPage(page: number, genOffset: number | null = null) {
    this.loading.set(true);
    const offset = genOffset !== null ? genOffset + page * this.pageSize : page * this.pageSize;
    const gen = this.generations.find(g => g.offset === this.selectedGen());
    const limit = gen ? Math.min(this.pageSize, gen.limit - page * this.pageSize) : this.pageSize;

    this.pokemonService.getList(limit > 0 ? limit : this.pageSize, offset).subscribe(resp => {
      this.total.set(resp.count);
      this.allItems.set(resp.results);
      this.loadDetails(resp.results);
    });
  }

  loadDetails(items: PokemonListItem[]) {
    const detailRequests = items.map(item => {
      const id = this.pokemonService.extractIdFromUrl(item.url);
      return this.pokemonService.getDetail(id.toString());
    });

    let loaded = 0;
    const results: PokemonCard[] = new Array(items.length);

    detailRequests.forEach((req, i) => {
      req.subscribe({
        next: detail => {
          results[i] = {
            id: detail.id,
        ...
        error: () => {
          loaded++;
          if (loaded === items.length) {
            this.cards.set(results.filter(Boolean));
            this.loading.set(false);
          }
        }
      });
  ...
  setGeneration(offset: number | null) {
    ...
    if (offset !== null) {
      const gen = this.generations.find(g => g.offset === offset)!;
      this.total.set(gen.limit);
  ...
  prevPage() {
    if (this.currentPage() > 0) {
      ...
      this.loadPage(this.currentPage(), gen);
    }
  }
